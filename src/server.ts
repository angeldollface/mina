...
export class ExtendedPage {
  ...
  constructor(route: URLPattern, pageName: string, pageContents: string) {
    this.route = route;
...
export function extendedPagesFromApp(app: App): Array<ExtendedPage> {
  ...
  for (let entry of Array.from(app.routes.entries())) {
    ...
    let extendedPage: ExtendedPage = new ExtendedPage(
      pattern,
      pageName,
      pageContents,
    );
...
export async function serveApp(
  ...
  const handler = (request: Request): Response => {
    ...
    if (!respBody) return new Response("Page not found!", {status: 404}) 
    return new Response(respBody, { status: 200, headers: {"Content-type": "text/html"} });
  };
  try {
